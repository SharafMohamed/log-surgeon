...
auto get_intersect_for_query(ByteLexer& lexer, std::string const& search_string) -> void {
    std::string proccessed_search_string;
    // Replace all * with .*
    for (char const& c : search_string) {
        if (c == '*') {
            proccessed_search_string.push_back('.');
        }
        proccessed_search_string.push_back(c);
    }
    log_surgeon::Schema schema2;
    schema2.add_variable("search", proccessed_search_string, -1);
    ...
    for (auto const& schema_type : schema_types) {
        std::cout << lexer.m_id_symbol[schema_type] << std::endl;
    }
}
